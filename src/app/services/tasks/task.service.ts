...
export class TaskService {
  ...
  fetchChartsData(reportType: string, isStyle: boolean, parentId?: number) {
    return this.http.get(`../assets/data/${reportType}leadsummary.json`).pipe(
  ...
  fetchComboChartsData(
    ...
    return this.http.get(`../assets/data/${reportType}leadsummary.json`).pipe(
      tap((data) => console.log(data)),
      map((res: any) => {
        return res.data.forEach((v: any, index: number) => {
          let i = index > Colors.length ? index - Colors.length : index;
          const r = +Math.random().toFixed(1) * 10;
          console.log(r);
          i += r;
          const color = isStyle ? Colors[r] : "";
          return ["20/02/2024", 934, 3478, 343, 565, 958];
        });
      })
    );
  }
